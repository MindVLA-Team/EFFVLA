...
# Implements KV-cache attention sharing with the VLM backbone
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        ).transpose(1, 2)                           # [B, num_heads,    q_len, head_dim]

        key_states = key_states.view(
            batch_size, q_len, self.num_key_value_heads, self.head_dim
        ).transpose(1, 2)                           # [B, num_kv_heads, q_len, head_dim]
...
class DiTLayer(nn.Module):
    ...
    def forward(
        ...
        (shift_attn, scale_attn, gate_attn,
         shift_mlp,  scale_mlp,  gate_mlp) = t_embeddings.chunk(6, dim=-1)
...
class KVSharedDiT(nn.Module):
    """DiT with KV cache sharing with the VLM"""
    def __init__(
        self,
        hidden_size: int,
        num_layers: int,
        ...
        print("t_embedder hidden_size:", hidden_size)
...
class KVSharedFlowmatchingActionHeadConfig(PretrainedConfig):
    ...
    vlm_kv_layer_offset: int = field(default=None, metadata={"help": "Offset from end of VLM layers. If None, uses last dit_num_layers VLM layers."})
...
class KVSharedFlowmatchingActionHead(nn.Module):
    ...
    Key differences from the base flow-matching action head:
    - Uses KV cache sharing instead of cross-attention
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if action_config.num_target_vision_tokens > 0:
            # Future tokens
            self.future_tokens = nn.Embedding(action_config.num_target_vision_tokens, self.hidden_size)
    ...
    def _prepare_dit_input(
        ...
        """Prepare DiT input: state + future_tokens + action"""
    ...
    def forward(
        ...
        dit_position_ids = torch.arange(
            dit_len, device=device, dtype=torch.long
        ).unsqueeze(0).expand(batch_size, -1) + vlm_max_pos + 1
        ...
        if self.vlm_kv_layer_offset is not None:
            ...
            if vlm_kv_start_idx < 0:
                raise ValueError(
                    f"dit_num_layers ({self.dit_num_layers}) cannot exceed VLM layers ({vlm_num_layers})"
                )
        ...
        selected_vlm_kv = extract_kv_from_cache(
            vlm_past_key_values, 
            start_idx=vlm_kv_start_idx, 
            num_layers=self.dit_num_layers
        )
    ...
    def predict_action(
        ...
        for step in range(num_steps):
            t_continuous = torch.full(
                size=(batch_size,), fill_value=step / num_steps,
...
def get_kv_shared_action_model(config=None):
    ...
    """
    return KVSharedFlowmatchingActionHead(
